...
export const generateVoxel = ({ colorPool, color, ambientColor, size, steps, padding, materials, camera, scene, groups }) => {
  const createVaderMesh = (material) => {
    return new THREE.Mesh(new THREE.CubeGeometry(1, 1, 1), material)
  }
  ...
  const VaderMesh = (obj = {}) => {
    ...
    for (let j = 0; j < size; j += steps) {
      ...
      for (let i = 0; i < size / 2; i += steps) {
        let c = Math.random(1) > 0.5 ? false : true
        col[j][i] = c
...
function VoxelVader({
    ...
    if (groups.length) {
      for (let i = 0; i < groups.length; i++) {
        if (groups[i].isGlowing) {
          groups[i].material.uniforms.viewVector.value = new THREE.Vector3().subVectors(camera.position, voxelVaderMesh.position)
        }
      }
    }
    return
  ...
  return <primitive object={voxelMesh} name={'VoxelVader'} ref={meshRef} position={position} rotation={[0, 0, 0]} scale={[0.25, 0.25, 0.25]} />
}
